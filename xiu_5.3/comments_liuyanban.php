...
<h3 class="title" id="comments">
	...
	<strong><?php echo _hui('comment_title') ?></strong>
</h3>
			...
			<?php
				if ( is_user_logged_in() ) {
					...
					if( get_option('require_name_email') && !empty($comment_author_email) ){
						printf($comment_author.' <span>'.__('既然来了就说点什么吧~~~', 'haoui').'</span> &nbsp; <a class="switch-author" href="javascript:;" data-type="switch-author" style="font-size:12px;">'.__('换个身份', 'haoui').'</a>');
					}else{
						printf('');
					}
				}
