...
if (_hui('layout') !== 'ui-c2') {
	if (function_exists('register_sidebar')) {
		$widgets = array (
			'sitesidebar' => __('全站侧栏',
			'haoui'
		), 'sidebar' => __('首页侧栏', 'haoui'), 'othersidebar' => __('分类/标签/搜索页侧栏', 'haoui'), 'postsidebar' => __('文章页侧栏', 'haoui'), 'pagesidebar' => __('页面侧栏', 'haoui'),);
...
function hui_nav_menu($class = 'nav', $location = 'nav') {
	echo '<ul class="' . $class . '"><li class="navmore"></li>' . str_replace("</ul></div>", "", preg_replace("/<div[^>]*><ul[^>]*>/", "", wp_nav_menu(array (
 ...
 * ====================================================
*/
function hui_logo($class = 'logo', $tag = array (
	'h1',
	'div'
)) {
...
function hui_posts_related($title = '', $limit = 8, $model = 'thumb') {
	...
	if ($posttags) {
		...
		$args = array (
			'post_status' => 'publish',
			'tag_slug__in' => explode(',',
			$tags
		), 'post__not_in' => explode(',', $exclude_id), 'ignore_sticky_posts' => 1, 'orderby' => 'comment_date', 'posts_per_page' => $limit);
		query_posts($args);
		while (have_posts()) {
			the_post();
	...
	if ($i < $limit) {
		...
		foreach (get_the_category() as $cat)
			$cats .= $cat->cat_ID . ',';
		$args = array (
			'category__in' => explode(',',
			$cats
		), 'post__not_in' => explode(',', $exclude_id), 'ignore_sticky_posts' => 1, 'orderby' => 'comment_date', 'posts_per_page' => $limit - $i);
		query_posts($args);
		while (have_posts()) {
			the_post();
			echo '<li><a' . hui_target_blank() . ' href="' . get_permalink() . '">';
...
function hui_get_thumbnail($single = true, $must = true) {
	...
	if (has_post_thumbnail()) {
		...
		foreach ($images as $src) {
			...
			if (/*( $etype == 'more' && $counter >= 12 && $i >= 12 ) ||
						( $etype == 'multi' && ($counter >= 8 && $i >= 8) || ($counter >= 4 && $i >= 4) || ($counter > 0 && $counter < 4 && $i >= 1) ) ||
						( $etype == 'four' && $counter >= 4 && $i >= 4 ) ||
						( $etype == 'thumb' && $counter >= 1 && $i >= 1 )*/

				 ($etype == 'more' && $counter >= 12 && $i >= 12) || ($etype == 'multi' && $counter >= 8 && $i >= 8) || ($etype == 'four' && $counter >= 4 && $i >= 4) || ($counter >= 12 && $i >= 12) || ($counter >= 8 && $counter < 12 && $i >= 8) || ($counter >= 4 && $counter < 8 && $i >= 4) || ($counter > 0 && $counter < 4 && $i >= 1)) {
				break;
			}
		}
	}
...
function timeago($ptime) {
	...
	if ($etime < 1)
		return __('刚刚', 'haoui');
	$interval = array (
		12 * 30 * 24 * 60 * 60 => __('年前',
		'haoui'
	) . ' (' . date('Y-m-d', $ptime) . ')', 30 * 24 * 60 * 60 => __('个月前', 'haoui') . ' (' . date('m-d', $ptime) . ')', 7 * 24 * 60 * 60 => __('周前', 'haoui') . ' (' . date('m-d', $ptime) . ')', 24 * 60 * 60 => __('天前', 'haoui'), 60 * 60 => __('小时前', 'haoui'), 60 => __('分钟前', 'haoui'), 1 => __('秒前', 'haoui'));
...
function hui_load_scripts() {
	if (!is_admin()) {
		...
		wp_enqueue_script('_bootstrap', _hui('js_outlink') ? $jss[_hui('js_outlink')]['bootstrap'] : THEME_URI . '/js/bootstrap.js', array (
			'jquery'
		), THEME_VERSION, true);
