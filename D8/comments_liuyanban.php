...
<div id="respond" class="no_webshot">
	...
	<h3 class="queryinfo">
		<?php printf('您必须 <a href="%s">登录</a> 才能发表留言！', wp_login_url( get_permalink() ) );?>
	</h3>
	...
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
		...
		<div class="comt-title">
			...
			<?php
				if ( is_user_logged_in() ) {
					printf($user_identity.'<span>发表我的留言</span>');
				}else{
					if( get_option('require_name_email') && !empty($comment_author_email) ){
						printf($comment_author.' <span>发表我的留言</span> &nbsp; <a class="switch-author" href="javascript:;" data-type="switch-author" style="font-size:12px;">换个身份</a>');
					}else{
						printf('发表我的留言');
					}
				}
		...
		<div class="comt">
			<div class="comt-box">
				...
				<div class="comt-ctrl">
					<button class="btn btn-primary pull-right" type="submit" name="submit" id="submit" tabindex="5"><i class="icon-ok-circle icon-white icon12"></i> 提交留言</button>
